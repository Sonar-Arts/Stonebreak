package com.openmason.engine.rendering.viewer.camera;
 ...
 */
public class ViewerCamera implements com.openmason.engine.rendering.api.IRenderCamera {

    private static final Logger logger = LoggerFactory.getLogger(ViewerCamera.class);
    ...
    public Vector3f getTarget() {
        return activeController.getTarget();
    }

    /**
     * Sets the point the camera looks at, if the active controller supports it
     * (the arcball camera moves its pivot; first-person ignores it).
     *
     * <p>Marks the view matrix dirty so the change is visible on the next frame
     * without waiting for another camera interaction.
     */
    public void setTarget(Vector3f target) {
        activeController.setTarget(target);
        viewMatrixDirty = true;
    }
