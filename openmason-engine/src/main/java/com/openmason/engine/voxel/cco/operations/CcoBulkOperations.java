...
public final class CcoBulkOperations {
     ...
     */
    public int fill(int startX, int startY, int startZ, int endX, int endY, int endZ, IBlockType block) {
        ...
        int changedCount = 0;
        IBlockType[][][] array = blocks.getUnderlyingArray();
     ...
     * @return Number of blocks changed
     */
    public int replaceAll(IBlockType from, IBlockType to) {
    ...
    /**
     * Clears entire chunk (sets all to null/air).
     *
     ...
     * @return Number of blocks changed
     */
    public int copyFrom(IBlockType[][][] source) {
        if (source == null) {
            return 0;
        }
        ...
        for (int x = 0; x < sizeX; x++) {
            for (int y = 0; y < sizeY; y++) {
                for (int z = 0; z < sizeZ; z++) {
                    IBlockType sourceBlock = source[x][y][z];
     ...
     * @return Number of blocks changed
     */
    public int fillSphere(int centerX, int centerY, int centerZ, int radius, IBlockType block) {
        if (radius <= 0) {
            return 0;
        }
