...
public class ChunkMeshBuildingPipeline {
    ...
    public ChunkMeshBuildingPipeline(WorldConfiguration config, ChunkStateManager stateManager, ChunkErrorReporter errorReporter) {
        this.config = config;
    ...
    public void scheduleConditionalMeshBuild(Chunk chunk) {
        ...
        synchronized (chunk) {
            if (!stateManager.isReadyForMeshGeneration(chunk)) {
                return;
            }
            ...
            stateManager.markMeshGenerationInProgress(chunk);
    ...
    private void processMeshBuildTask(World world, Chunk chunkToProcess) {
        ...
        try {
            ...
            stateManager.markMeshGenerationComplete(chunkToProcess);
    ...
    public void requeueFailedChunks() {
        ...
        while ((failedChunk = chunksFailedToBuildMesh.poll()) != null) {
            stateManager.resetMeshGenerationState(failedChunk);
