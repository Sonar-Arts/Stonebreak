 ...
 * layer (unit-square UVs) with a per-vertex layer index. Submerged cells paint
 * a sea-level sheet (waterHeight=0, alphaFlag=0) so the shader's opaque branch
 * draws them without wave displacement.
 */
public final class FastLodMesher {
    ...
    public MmsMeshData build(FastLodChunkData data) {
        ...
        for (int ix = 0; ix < cellsPerAxis; ix++) {
            for (int iz = 0; iz < cellsPerAxis; iz++) {
                ...
                w.topQuad(wx, topY, wz, cellSize, topLayer);
    ...
    private void emitSkirtIfLower(QuadWriter w, FastLodChunkData data,
        ...
        w.skirtQuad(wx, wz, cellSize, dx, dz, topY, nTopY, sideLayer);
    }
    ...
    private static final class QuadWriter {
        ...
        void topQuad(float wx, float y, float wz, int cellSize, int layer) {
            float x1 = wx + cellSize;
            float z1 = wz + cellSize;
            int v0 = pushVert(wx, y, wz, 0f, 0f, 0, 1, 0, 0f, layer);
                    pushVert(x1, y, wz, 1f, 0f, 0, 1, 0, 0f, layer);
                    pushVert(x1, y, z1, 1f, 1f, 0, 1, 0, 0f, layer);
                    pushVert(wx, y, z1, 0f, 1f, 0, 1, 0, 0f, layer);
            pushQuadIndices(v0);
        }

        /** Skirt face on one cell edge descending from topY to bottomY. */
        void skirtQuad(float wx, float wz, int cellSize,
                       int dx, int dz, int topY, int bottomY, int layer) {
            float x1 = wx + cellSize;
            float z1 = wz + cellSize;
            float fTop = topY, fBot = bottomY;
            if (dx > 0) {
                int v0 = pushVert(x1, fTop, wz, 0f, 0f, 1, 0, 0, 0f, layer);
                        pushVert(x1, fTop, z1, 1f, 0f, 1, 0, 0, 0f, layer);
                        pushVert(x1, fBot, z1, 1f, 1f, 1, 0, 0, 0f, layer);
                        pushVert(x1, fBot, wz, 0f, 1f, 1, 0, 0, 0f, layer);
                pushQuadIndices(v0);
            } else if (dx < 0) {
                int v0 = pushVert(wx, fTop, z1, 0f, 0f, -1, 0, 0, 0f, layer);
                        pushVert(wx, fTop, wz, 1f, 0f, -1, 0, 0, 0f, layer);
                        pushVert(wx, fBot, wz, 1f, 1f, -1, 0, 0, 0f, layer);
                        pushVert(wx, fBot, z1, 0f, 1f, -1, 0, 0, 0f, layer);
                pushQuadIndices(v0);
            } else if (dz > 0) {
                int v0 = pushVert(x1, fTop, z1, 0f, 0f, 0, 0, 1, 0f, layer);
                        pushVert(wx, fTop, z1, 1f, 0f, 0, 0, 1, 0f, layer);
                        pushVert(wx, fBot, z1, 1f, 1f, 0, 0, 1, 0f, layer);
                        pushVert(x1, fBot, z1, 0f, 1f, 0, 0, 1, 0f, layer);
                pushQuadIndices(v0);
            } else {
                int v0 = pushVert(wx, fTop, wz, 0f, 0f, 0, 0, -1, 0f, layer);
                        pushVert(x1, fTop, wz, 1f, 0f, 0, 0, -1, 0f, layer);
                        pushVert(x1, fBot, wz, 1f, 1f, 0, 0, -1, 0f, layer);
                        pushVert(wx, fBot, wz, 0f, 1f, 0, 0, -1, 0f, layer);
                pushQuadIndices(v0);
            }
        }
        ...
        void axisAlignedQuad(float x0, float y0, float z0,
            ...
            int v0 = pushVert(x0, y0, z0, 0f, 0f, nx, ny, nz, alphaFlag, layer);
                    pushVert(x1, y1, z1, 1f, 0f, nx, ny, nz, alphaFlag, layer);
                    pushVert(x2, y2, z2, 1f, 1f, nx, ny, nz, alphaFlag, layer);
                    pushVert(x3, y3, z3, 0f, 1f, nx, ny, nz, alphaFlag, layer);
            pushQuadIndices(v0);
        }

        private int pushVert(float x, float y, float z,
                             float u, float v,
                             float nx, float ny, float nz, float alphaFlag, int layer) {
            int vi = vertCount;
            int p3 = vi * 3, p2 = vi * 2;
