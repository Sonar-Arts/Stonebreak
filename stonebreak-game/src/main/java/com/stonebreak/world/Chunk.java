...
public class Chunk {
    ...
    // Save/Load and modification tracking
    private boolean isDirty = false; // True if chunk has been modified since last save
    private boolean generatedByPlayer = false; // True if chunk modifications came from player actions
    private java.time.LocalDateTime lastModified = null; // When chunk was last modified
    ...
    public void setBlock(int x, int y, int z, BlockType blockType) {
        ...
        // Check if this is actually changing the block
        BlockType currentBlock = blocks[x][y][z];
        if (currentBlock != blockType) {
            blocks[x][y][z] = blockType;
            markDirty(false); // Default to not player-generated for general setBlock calls
        }
    }
    
    /**
     * Sets the block type at the specified local position, with player modification tracking.
     */
    public void setBlock(int x, int y, int z, BlockType blockType, boolean playerModified) {
        if (x < 0 || x >= World.CHUNK_SIZE || y < 0 || y >= World.WORLD_HEIGHT || z < 0 || z >= World.CHUNK_SIZE) {
            return;
        }
    ...
    // Save/Load and modification tracking methods
    
    /**
     * Marks this chunk as dirty (modified and needing to be saved).
     */
    public void markDirty(boolean playerModified) {
        this.isDirty = true;
        this.lastModified = java.time.LocalDateTime.now();
        if (playerModified) {
            this.generatedByPlayer = true;
            System.out.println("[CHUNK-DIRTY] Chunk (" + getChunkX() + "," + getChunkZ() + ") marked dirty by PLAYER modification");
        } else {
            System.out.println("[CHUNK-DIRTY] Chunk (" + getChunkX() + "," + getChunkZ() + ") marked dirty by WORLD generation");
        }
    }
    
    /**
     * Marks this chunk as clean (saved and up-to-date).
     */
    ...
    /**
     * Checks if this chunk has been modified by player actions.
     */
    ...
    /**
     * Gets the last modification time for this chunk.
     */
    ...
    /**
     * Sets the dirty state (used when loading from save).
     */
    ...
    /**
     * Sets whether this chunk was generated by player actions (used when loading from save).
     */
    ...
    /**
     * Sets the last modification time (used when loading from save).
     */
    ...
    /**
     * Gets whether features (trees, flowers, etc.) have been populated in this chunk.
     */
    ...
    /**
     * Sets whether features have been populated (used when loading from save).
     */
   /** public void setFeaturesPopulated(boolean featuresPopulated) {
    *    this.featuresPopulated = featuresPopulated;
    *}
