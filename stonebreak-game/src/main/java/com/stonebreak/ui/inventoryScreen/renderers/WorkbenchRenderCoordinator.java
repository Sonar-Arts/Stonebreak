...
/**
 * Coordinates rendering phases for the workbench screen.
 * Uses workbench-specific layout (3x3 crafting grid) and "Workbench" title.
 * Follows Single Responsibility Principle by handling only workbench rendering coordination.
 */
public class WorkbenchRenderCoordinator {
    ...
    private void renderCraftingSection(InventoryLayoutCalculator.InventoryLayout layout) {
        ...
        InventoryPanelRenderer.drawInventoryTitle(uiRenderer, layout.panelStartX + layout.inventoryPanelWidth / 2,
                                                craftingTitleY, "Workbench");

        // Render crafting input slots (3x3 grid)
        renderCraftingInputSlots(layout);

        // Render crafting arrow
    ...
    private void renderCraftingInputSlots(InventoryLayoutCalculator.InventoryLayout layout) {
        ...
        for (int i = 0; i < InventoryLayoutCalculator.getWorkbenchCraftingInputSlotsCount(); i++) {
            int r = i / InventoryLayoutCalculator.getWorkbenchCraftingGridSize();
            int c = i % InventoryLayoutCalculator.getWorkbenchCraftingGridSize();
    ...
    private void checkHover(ItemStack itemStack, int slotX, int slotY) {
        Vector2f mousePos = inputHandler.getMousePosition();
        boolean isHovering = mousePos.x >= slotX && mousePos.x <= slotX + InventoryLayoutCalculator.getSlotSize() &&
                           mousePos.y >= slotY && mousePos.y <= slotY + InventoryLayoutCalculator.getSlotSize();
