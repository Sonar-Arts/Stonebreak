...
import com.openmason.main.systems.rendering.model.io.omo.OmoPartRestorer;
...
public class ModelOperationService {
            ...
            if (loadedModel != null) {
                ...
                if (viewport != null) {
                    if (meshData != null && meshData.hasCustomGeometry()) {
                        ...
                        Map<Integer, Integer> restoredFaceIds = null;
                        if (partEntries != null && !partEntries.isEmpty()) {
                            ...
                            OmoPartRestorer.Result restored = OmoPartRestorer.restore(
                                    viewport.getPartManager(), meshData, partEntries, bonesForRestore);
                            restoredFaceIds = restored.faceIds();
                            if (!restored.skippedParts().isEmpty()) {
                                logger.warn("Skipped corrupt OMO parts: {}", restored.skippedParts());
                            }
                        ...
                        if (faceTextureData != null && !faceTextureData.mappings().isEmpty()) {
                            restoreFaceTextureData(faceTextureData, materialTextures, restoredFaceIds);
                        }
    ...
    private void restoreFaceTextureData(OMOFormat.FaceTextureData faceTextureData,
                                         Map<String, byte[]> materialTextures, Map<Integer, Integer> restoredFaceIds) {
        if (viewport == null) {
            return;
        }
        ...
        for (OMOFormat.FaceMappingEntry mapEntry : faceTextureData.mappings()) {
            Integer faceId = restoredFaceIds == null ? mapEntry.faceId()
                    : restoredFaceIds.get(mapEntry.faceId());
            if (faceId == null) continue; // texture belonged to a skipped corrupt part
            ...
            FaceTextureMapping mapping = new FaceTextureMapping(
                    faceId, mapEntry.materialId(), uvRegion, uvRotation,
                    mapEntry.autoResize());
