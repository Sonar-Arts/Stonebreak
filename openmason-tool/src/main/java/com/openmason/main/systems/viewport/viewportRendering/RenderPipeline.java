...
public class RenderPipeline {
    ...
    // Face data caching to prevent unnecessary updates (same pattern as vertices/edges)
    ...
    public void render(ViewportUIState viewportState, RenderingState renderingState, TransformState transformState) {
        try {
            ...
            if (viewportState.getShowVertices().get()) {
                ...
                renderFaces(renderingState, transformState);
            }
    ...
    /**
     * Render faces pass (complements vertex and edge rendering for mesh visualization).
     */
    private void renderFaces(RenderingState renderingState, TransformState transformState) {
        try {
            ...
            // Enable face renderer
            if (!faceRenderer.isEnabled()) {
                faceRenderer.setEnabled(true);
                logger.debug("Face renderer enabled");
            }

            // Render faces based on current rendering mode (mirrors vertex/edge rendering)
            switch (renderingState.getMode()) {
                case BLOCK_MODEL:
                    // Editable .OMO block model rendering (simple cube)
                    Collection<ModelDefinition.ModelPart> cubeParts = createCubeParts();
